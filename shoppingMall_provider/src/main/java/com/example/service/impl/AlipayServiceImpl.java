...
public class AlipayServiceImpl implements AlipayService {
    ...
    public AlipayTradePagePayResponse pay(String payOrderNo, Double totalAmount, String payment, User user, String... skuid) {
        AlipayClient alipayClient = new DefaultAlipayClient(alipayConfiguration.getGatewayUrl(),
                alipayConfiguration.getApp_id(),
                alipayConfiguration.getMerchant_private_key(),
                alipayConfiguration.getFormat(),
                alipayConfiguration.getCharset(),
                alipayConfiguration.getAlipay_public_key(),
...
//        bizContent.put("cert_no", user.getIdCard());//买家证件号
        ...
        PayRequest.setReturnUrl(alipayConfiguration.getReturn_url()); //设置同步回调地址
        PayRequest.setNotifyUrl(alipayConfiguration.getNotify_url()); //设置异步回调地址
    ...
    public String verify(Map<String, String> params, String outOrderNo) {
        ...
        try {
            isSign = AlipaySignature.rsaCheckV1(params, alipayConfiguration.getAlipay_public_key(), alipayConfiguration.getCharset(),alipayConfiguration.getSign_type());
        ...
        if (!sellerId.equals(alipayConfiguration.getSellerPid())) {
            log.error(ConstantUtil.VERIFY_FAIL_LOG);
            return result;
        }

        String appId = params.get("app_id");
        if (!appId.equals(alipayConfiguration.getApp_id())) {
            log.error(ConstantUtil.VERIFY_FAIL_LOG);
            return result;
        }
